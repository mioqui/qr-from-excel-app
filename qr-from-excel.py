...
st.title("🧾 Generador de QRs desde Excel")
...
if uploaded_file:
    ...
    if seleccion is not None:
        ...
        texto = " | ".join([str(fila[col]) for col in df.columns[:3]])  # Mostrar primeras 3 columnas como texto
    ...
    if st.button("📦 Generar ZIP con QRs"):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, row in df.iterrows():
                texto = " | ".join([str(row[col]) for col in df.columns[:3]])  # Texto visible arriba del QR
